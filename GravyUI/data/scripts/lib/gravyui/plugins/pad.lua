...
return function(node, a, b, c, d)
    ...
    if d ~= nil then
        if math.abs(a) <= 1 then a = a * node.rect.width end
        if math.abs(b) <= 1 then b = b * node.rect.height end
        if math.abs(c) <= 1 then c = c * node.rect.width end
        if math.abs(d) <= 1 then d = d * node.rect.height end
        topLeft = vec2(a, b)
